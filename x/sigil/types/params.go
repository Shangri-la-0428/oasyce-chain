...
func (p Params) Validate() error {
	if p.DormantThreshold <= 0 {
		return ErrInvalidParams.Wrap("dormant_threshold must be positive")
	}
	if p.DissolveThreshold <= 0 {
		return ErrInvalidParams.Wrap("dissolve_threshold must be positive")
	}
	if p.DissolveThreshold <= p.DormantThreshold {
		return ErrInvalidParams.Wrap("dissolve_threshold must exceed dormant_threshold")
	}
	if p.SubmitWindow <= 0 {
		return ErrInvalidParams.Wrap("submit_window must be positive")
	}
	return nil
}
