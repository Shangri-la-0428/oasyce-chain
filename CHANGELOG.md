...
- **Data Agent** — `scripts/data_agent.py` autonomous data asset registration daemon. Integrates scan/classify/privacy pipeline (formerly DataVault, now `oasyce-agent` via oasyce-sdk); auto-registers safe assets (risk=safe/low) on-chain with zero human intervention. Supports `--dry-run`, `--once`, extension allowlist, path exclusion, and configurable risk threshold. Health endpoint at `/health`.
